...
The \textit{painful} part  of writing formatting code is  to take into
account the  missing fields  without multiplying explicit  checks that
would quickly be boring. In addition,  it is important to make it easy
to factor  out common operations,  like formatting a list  of authors,
and reuse them in different contexts.

\pyblio  provides a \textit{domain  specific language}  that addresses
these problems. However,  it is not intended as  a complete formatting
language, so you cannot use it  for instance to lay out your citations
in a complete  HTML web page (but this  specific part is comparatively
easy).
...
In this example,  the \py{authors} variable is composed  by taking all
the  values in the  author field  (\py{all('author')}), and  pass them
through the  \py{lastFirst} transformation, which will  format them as
\textit{Last Name,  First Name}. The \py{Person} module contains
other formatting variants for person names.
...
alternative  operator  can be  used  everywhere  a  definition can  be
invalid.

You can then group these fields together, possibly adding some style
...
The \py{join} operator  with take the parts between  square braces and
link them  together with the text  specified in parameter,  a comma in
...
operator). In addition, the title is enclosed in a bold \py{B} tag.
...
actual formatting of specific records can be a fast process.
...
You still to get a definitive result, as you still need to select the
